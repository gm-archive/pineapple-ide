...
public final class Parser {
    ...
    //<editor-fold defaultstate="collapsed" desc="Named class">
    public CompilerReturnData<ClassConstant> parseClass(int i)
    throws ParserException{
        final Token init = demandToken(i++);
        Token t = init;
        if(init.getType()==TokenType.WORD){
            if(!init.getContent().equals("magic")){ //Don't ask
                return null;
            }
        ...
        if(name.getType()!=TokenType.WORD){
            return null; //anonymous classes are parsed elsewhere
        }
        ...
        if(
                (next.getType()==TokenType.OPERATOR&&next.getOperator()==Operator.COLON)
                || (next.getType()==TokenType.KEYWORD&&next.getKeyword()==Keyword.EXTENDS)){
        ...
        for(next=demandToken(i);
               !(next.getType()==TokenType.OPERATOR&&next.getOperator()==Operator.CBC)
        ;next=demandToken(i)){
    ...
    throws ParserException{
        ...
        if(k==Keyword.PRIVATE||k==Keyword.PROTECTED||k==Keyword.PUBLIC){
            ...
            //Currently, access modifiers are mandatory.
            //This may change in the future.
            //However, to avoid behavior changes, it's best to
            //have the feature disabled instead of having wrong
            //behavior.
            return null;
        }
    ...
    public CompilerReturnData<Function> parseMethod(int i)
    throws ParserException{
        final Token init = demandToken(i++);
        Token t = init;
        if(t.getType()!=TokenType.KEYWORD){
        ...
        if(stmt==null){
            fastError(true, t,
                    "Expecting statement in constructor");
        }
        i = stmt.i;
        c.stmt = stmt.leaf;
    ...
    public CompilerReturnData<Statement> parseStatement(final int i)
            throws ParserException{
    ...
    public CompilerReturnData<BlockStatement> parseBlock(int i)
    throws ParserException{
    ...
    public CompilerReturnData<Expression> parseExpressionStatement(final int i)
    throws ParserException{
    ...
    public CompilerReturnData<IfStatement> parseIfStatement(int i)
    throws ParserException{
    ...
    public CompilerReturnData<Statement> parseReturnStatement(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="Argument list">
    public CompilerReturnData<ArgumentList> parseArgumentList(int i)
    throws ParserException{
        Token t;
        ...
        for(t = demandToken(++i); t.getType()!=TokenType.OPERATOR; t = demandToken(++i)){
            ...
            if(t.getType()==TokenType.OPERATOR&&t.getOperator()==Operator.COLON){
                ...
                if(requiresId){
                    fastError(true, t.getLine(),
                            t.getCol(), "Numbers are not valid argument names");
                    throw new ParserException();
                }
                ...
                try{
                    ...
                    fastError(true, t.getLine(),
                            t.getCol(), "Can not inference argument id.");
                    throw new ParserException();
                }
            }
    ...
    //<editor-fold defaultstate="collapsed" desc="Var/Const declaration">
    public CompilerReturnData<VariableDeclaration> parseVariableDeclaration(int i)
    throws ParserException{
        Token t;
        ...
        if(decl.arguments.isEmpty()){
            fastError(true,
                    t.getLine(), t.getCol(), "Empty variable/constant declaration");
        }
    ...
    public CompilerReturnData<StringConstant> parseString(final int i)
    throws ParserException{
        final Token t = demandToken(i);
    ...
    public CompilerReturnData<NumericConstant> parseNumber(final int i)
    throws ParserException{
        final Token t = demandToken(i);
    ...
    //<editor-fold defaultstate="collapsed" desc="parseConstant">
    public CompilerReturnData<Constant> parseConstant(final int i)
    throws ParserException{
    ...
    //These functions are separated for operator precedence reasons.

    //<editor-fold defaultstate="collapsed" desc="Trivial left reference">
    public CompilerReturnData<Reference> parseTrivialLeftReference(final int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="Primitive">
    public CompilerReturnData<Expression> parsePrimitive(final int i)
    throws ParserException{
        //This actually does very little, and yet it is very useful.
        //It mostly redirects the parser to other functions.

        //Here, we try to parse some stuff like constants, simple references,
        //statements in parenthesis like (1 + 2), constructors and arrays.
        ...
        //TODO: Constructors - new XYZ(1, 2, 3);
        //TODO: Arrays - [1, 2, 3]
    ...
    public CompilerReturnData<Expression> parseReference(int i)
    throws ParserException{
        //This should deal with ((x.y).z())()[3](1, 2, 3).a

        //To parse the example above, we use the following strategy:
        //1. We obtain a "primitive" (MANDATORY)
        //2. We then attempt to get(we may or not be able to) the following:
        //  a) A function reference a()
        //  b) A composed reference a.b
        //  c) An array reference a[b]
        //3. If 2. was successful, try it again
        //   Otherwise, return the current result.

        final CompilerReturnData primitive = parsePrimitive(i);
        if(primitive==null){
            //Primitive is *mandatory*
            return null;
        }
        ...
        while(true){
            ...
            //TODO: Parse composed reference

            //TODO: Parse array reference

            //If code reaches this point, then that means step 2. failed
            return new CompilerReturnData(--i, e);
        }
    }
    //</editor-fold>

    //<editor-fold defaultstate="collapsed" desc="Pre/Post increment/decrement">
    public CompilerReturnData<Expression> parsePrePostFixOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="logical not">
    public CompilerReturnData<Expression> parseLogicalNot(final int i)
    throws ParserException{
    ...
    public CompilerReturnData<Expression> parseMultOperator(int i)
    throws ParserException{
        ...
        for(;;){
            ...
            if(f==null){
                fastError(true, t,
                        "Unexpected statement on right side of operator");
            }
            i = f.i;
    ...
    //<editor-fold defaultstate="collapsed" desc="operators + -">
    public CompilerReturnData<Expression> parseSumOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operators << >>">
    public CompilerReturnData<Expression> parseBitwiseShiftOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operators < > <= >=">
    public CompilerReturnData<Expression> parseComparison1(int i)
    throws ParserException{
        ...
        for(;;){
            ...
            final Operator op = t.getOperator();
            if(op!=Operator.GREATER && op!=Operator.SMALLER
                    && op!=Operator.GTE && op != Operator.SME){
                break;
            }
    ...
    //<editor-fold defaultstate="collapsed" desc="operators == !=">
    public CompilerReturnData<Expression> parseComparison2(int i)
    throws ParserException{
    ...
    public CompilerReturnData<Expression> parseBitwiseAndOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operator |">
    public CompilerReturnData<Expression> parseBitwiseOrOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operator ^">
    public CompilerReturnData<Expression> parseBitwiseXorOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operator &&">
    public CompilerReturnData<Expression> parseLogicalAndOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operator ||">
    public CompilerReturnData<Expression> parseLogicalOrOperator(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="operator ?:">
    public CompilerReturnData<Expression> parseTernaryConditional(int i)
    throws ParserException{
    ...
    public CompilerReturnData<Expression> parseSingleValue(int i)
    throws ParserException{
    ...
    //<editor-fold defaultstate="collapsed" desc="Right list">
    public CompilerReturnData<Expression> parseRightList(int i)
    throws ParserException{
    ...
    public void fastError(boolean fatal, int line, int col, String message)
    throws ParserException{
        handler.errors.add(new CompilerError(fatal, line, col, message));
        throw new ParserException();
    }

    public void fastError(boolean fatal, Token t, String message)
    throws ParserException{
    ...
    public Token demandToken(final int i)
    throws ParserException{
        if(i>=handler.tokens.size()){
    ...
    public Token demandToken(final int i, final TokenType type)
    throws ParserException{
    ...
    public Token demandToken(final int i, final Operator operator)
    throws ParserException{
    ...
    public void demandType(final Token token, final TokenType type)
    throws ParserException{
        final TokenType foundType = token.getType();
        if(foundType!=type){
            handler.errors.add(
                    new CompilerError(
                    true, token.getLine(), token.getCol(), "Expected " + type + " found " + foundType));
            throw new ParserException();
        }
    }

    public void demandKeyword(final Token token, final Keyword key)
    throws ParserException{
        demandType(token, TokenType.KEYWORD);
        final Keyword foundKey = token.getKeyword();
        if(foundKey!=key){
            handler.errors.add(
                    new CompilerError(true, token.getLine(), token.getCol(),
                    "Expected " + key.getKeyword() + " found " + foundKey.getKeyword()));
            throw new ParserException();
        }
    }

    public void demandOperator(final Token token, final Operator operator)
    throws ParserException{
        ...
        if(foundOp!=operator){
            handler.errors.add(
                    new CompilerError(true, token.getLine(), token.getCol(),
                    "Expected " + operator.getStringRepresentation() + " found " + foundOp.getStringRepresentation()));
            throw new ParserException();
        }
    }
    //</editor-fold>
